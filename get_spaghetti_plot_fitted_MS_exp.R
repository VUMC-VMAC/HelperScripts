...
get_spaghetti_plot_fitted <- function(input,id_var,xaxis_var,yaxis_var,group_var,plottitle){
  ...
  dataset$xaxis2 <- dataset$xaxis * dataset$xaxis
	...
	beta3 <- 1:length(levels(data_temp$group))
	
	ctrl <- lmeControl(msMaxIter = 2000)
	...
	for (i in 1:length(levels(data_temp$group)))
	{
	  ...
	  #run mixed model
	  output  <- lme(yaxis ~ xaxis + xaxis2, data = data_temp2, random=~1+xaxis|id, na.action = na.omit, method = 'REML', control=ctrl)
	  a <- summary(output)
	  ...
	  beta3[i] <- a$tTable[3,1]
	}
	
	for (i in 1:length(levels(data_temp$group)))
	{
	  curve(beta1[i] + beta2[i]*x + beta3[i]*(x*x),from = min(data_temp$xaxis,na.rm=TRUE), to = max(data_temp$xaxis,na.rm=TRUE), add = TRUE, col=color[i],lwd = 18)
	}
