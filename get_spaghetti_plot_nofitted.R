...
get_spaghetti_plot_nofitted <- function(input,id_var,xaxis_var,yaxis_var,group_var,plottitle){
	data <- input
	...
	color <- c("red","black","gray","blue","green","yellow","orange")
	...
	plot(xaxis,yaxis, type = "n", xlab = label(data[,xaxis_column]), ylab = label(data[,yaxis_column]),main = plottitle,
	     axes = F, frame = T,xlim = c(round(min(xaxis,na.rm=TRUE)),round(max(xaxis,na.rm=TRUE))),ylim = c(round(min(yaxis,na.rm=TRUE)),round(max(yaxis,na.rm=TRUE))),cex.lab = 2, cex.main= 2.5)
	...
	for (i in 1:length(levels(data_temp$group)))
	{
	  ...
	  for(j in 1:length(uid)){points(xaxis[id == uid[j]], yaxis[id == uid[j]], type="o",cex=0.5,pch=20,col=color[i])}
	  detach(data_temp2)
	...
	#draw legend
	legend("topleft",c(levels(data_temp$group)), title = names(data[,group_column]), fill = c(color), cex = 1.5, bty='n', bg = "white")
